...
<div class="infinite-scroll wow fadeIn" > 
    ...
    @foreach ($all_last_posts['post'] as $item)               
       <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 margin-80px-bottom sm-margin-50px-bottom xs-margin-30px-bottom wow fadeIn height-317px" style="visibility: visible; animation-name: fadeInUp;">
           <div class="blog-post blog-post-style1 xs-text-center">
               <div class="blog-post-images overflow-hidden margin-25px-bottom xs-margin-15px-bottom">
                   <a href="{{url( $all_last_posts['lang'].'/'.$all_last_posts['folder'].'/'.$item->id.'/'.$item->date.'/'.urlencode($item->title))}}"> 
                       <img src="{{$item->oimg}}" alt="" data-no-retina="">
                   </a>
               </div>
               <div class="post-details">
                   <a href="{{url( $all_last_posts['lang'].'/'.$all_last_posts['folder'].'/'.$item->id.'/'.$item->date.'/'.urlencode($item->title))}}" class="post-title text-medium text-extra-dark-gray width-90 display-block md-width-100 height-82px"  style="font-weight:bold">{{$item->title}}</a>
