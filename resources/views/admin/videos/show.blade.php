...
@section('content')
<div class="container">
    <h3>Attache files to Video № {{$video->id}} <small><code> lang:{{$locale}}</code><small></h3>
          ...
          <tbody style="font-weight:600; font-size:13px">
            <tr>
              ...
              @else
                <td>{{$video->status}}</td>
              @endif
              <td><img src="{{$video->img}}" alt="" width="100px"></td>
            </tr>
          <tbody>
          <tfoot>
    ...
    <div class="row">
      <form action="{{ route('admin.document.uploadfile', $locale) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="col-md-3">  
            ...
            <input type="text" name="post_id" value="{{$video->id}}" hidden>
        ...
        <tfoot>
          <tr>
            <td colspan="3" class="text-center">
                {{ csrf_field() }}
              <input type="text" name="post_id" value="{{$video->id}}" readonly>
