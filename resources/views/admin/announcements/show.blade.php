...
@section('content')
<div class="container">
    <h3>Attache files to Announcement <small> № {{$announcement->id}} <code> lang:{{$locale}}</code><small></h3>
      ...
      <table class="table table-bordered table-striped table-hover">          
          ...
          <tbody style="font-weight:600; font-size:13px">
            <tr>
              ...
              <td>{{$announcement->a_duration}}</td>
              @if($announcement->status == 'not_published')
                <td class="alert alert-danger">{{$announcement->status}}</td>
              @else
                <td>{{$announcement->status}}</td>
              @endif
              <td><img src="{{$announcement->img}}" alt="" width="100px"></td>
            </tr>
          <tbody>
          <tfoot>
    ...
    <div class="row">
      <form action="{{ route('admin.document.uploadfile', $locale) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="col-md-3">  
            ...
            <input type="text" name="post_id" value="{{$announcement->id}}" hidden>
        ...
        <tfoot>
          <tr>
            <td colspan="3" class="text-center">
                {{ csrf_field() }}
              <input type="text" name="post_id" value="{{$announcement->id}}" readonly>
